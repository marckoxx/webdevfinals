...
const form = useForm(props.helmets);

const destroy = (id) => {
    if (confirm('Are you sure?')) {
        form.delete(route('helmets.destroy', id))
    }
}
...
watch(search, value => {
    router.get('/helmets', { search: value }, { preserveState: true });
});
                    ...
                    <div class="ms-auto  mt-n1 col-auto pe-3 mt-3 ">
                        <div class="row my-4">
                            <div class="col-sm-6 col-lg-6">
                                <Link :href="route('helmets.create')" class="btn rounded-pill m-1 fw-bold mx-5" style="background-color: #FFFFD0; border: 1px solid;">Add a Motorcycle Type</Link>
                            </div>
                        ...
                        <tbody>
                            <template v-for="helmet in helmets" :key="helmet.id">
                                <tr>
                                    <td>{{ helmet.id }}</td>
                                    <td>{{ helmet.name }}</td>
                                    <td>
                                        <Link :href="route('helmets.edit', helmet.id)" class="btn btn-primary rounded-pill">Edit</Link>
                                    </td>
                                    <td><button class="btn btn-danger rounded-pill" @click="destroy(helmet.id)">Delete</button></td>
                                </tr>
                            </template>
                        </tbody>
                    </table>
